...
#region ---Main
log -content "Gathering setup options: " -type 1 -append
# Check for AZURE subscritions
if ($useAzure) {
    
    if (get-command 'az' -ea SilentlyContinue) {
        log -content "Azure... " -type 1 -append
        $azureSignedIn = az ad signed-in-user show 2>$null 
    }
    else { log -content "Azure... " -type 2 -append }
    if ($azureSignedIn) {
...
if ($useGcloud) {
    if (get-command 'gcloud' -ea SilentlyContinue) { log -content "Gcloud... " -type 1 -append; $gcloudSignedIn = gcloud config get-value project  2>$null }
    else { log -content "Gcloud... " -type 2 --append }
    if ($gcloudSignedIn) {
        ...
        foreach ($i in $allProjects) {
            ...
            addProvider(New-object PSCustomObject -Property @{Provider = "GCP"; Name = $i.name; Identifier = $i.projectNumber; default = $defaultOption })
        }
    }
}
# Done getting options
log -content "Done!" -type 1
...
if ($providerDefault.count -eq 1) {
    ...
    addChoice(New-object PSCustomObject -Property @{Key = "target"; Description = "Change Script Target"; Current = " $($providerDefault[0].Provider) -> $($providerDefault[0].Name)" })
}
else {
...
write-output "`n$choices" | sort-object -property Option | format-table -Property Option, Description, Current
#endregion
