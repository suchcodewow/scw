...
| summarize dayofWeek=first(dayofWeek), warehouse = first(warehouse), rating = first(rating), by: orderid
...
| fields orderid,servicename, rating, review,status,content,loglevel
```
