---
title: Dynatrace Webapp Extension Installer
---

## Goal

Deploy the Dynatrace extension effortlessly to your Azure webapps without making pipeline changes or deployment adjustments.

This script will:

- run a preflight check on the options entered to ensure they are valid.
- collect necessary webapp data for the resource group or webapp you specify.
- query the webapp(s) for current state of the Dynatrace extension.
- provide you with a 'todo' list for your confirmation(i.e. which webapps need installs/config/updates).
- **stop** here by default. If implementation mode is enabled it will continue.
...
```bash
curl -L http://suchcodewow.io/manifests/awa.ps1 > dtextinst.ps1
```

### From Dynatrace you'll need:

1. Your dynatrace tenant URL.
1. A dynatrace PAAS token.

### From Azure you'll need:

1. the resource group containing your webapps.
1. (Optional) the name of the specific webapp where you want to deploy the extension.
1. the webapp service plan name.
...
| -plan                        | $iPlan=          | _only if generating an app_ | Only needed when generating blank apps for testing- not usually needed                       |
...
To see a todo list of what would happen in the 'webapp-group-3' resource group, run something like:

```bash
./dtextinst.ps1 -group webapp-group-3 -url https://12345678.live.dynatrace.com -token dt0c1.AlfeFigjkbE1VC33Csa
```

You should see a list of webapps and their current Dynatrace extension status.

Scoping to the single webapp 'BackendService2' would be something like:

```bash
./dtextinst.ps1 -group webapp-group-3 -webapp BackendService2 -url https://12345678.live.dynatrace.com -token dt0c1.AlfeFigjkbE1VC33Csa
```
