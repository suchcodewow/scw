...
The steps below summarize installation of the Dynatrace Operator. The only change from default is to set limits on cpu/memory to keep Azure costs down.
...
Find the section starting with **Optional: resource settings for OneAgent container** and remove the # signs so it looks like:
...
Follow a similar process for the section starting with **Optional: resource settings for ActiveGate container**. Change the memory limits from `1G` to `500M`. Remove the # signs so it looks like:
...
- Copy the entire contents of this file (usually with ctrl+a).

- In your azure cloud shell window, create and edit the same file with :
...
- Back in the Dynatrace tenant, click the **copy** button next to the commands and paste into your azure cloud shell window.
...
If one of the pods fails to start there could be several things going on. Checking the logs is a good start when troubleshooting. To look at the logs for a pod, use something like:
...
- Click on **Deployment Status** in the left dark menu bar.

You should see a new host running in your environment on the **OneAgents** page.

By clicking on **ActiveGates** you should also see a new activegate.
