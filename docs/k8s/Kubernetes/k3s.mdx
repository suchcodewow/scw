...
Login to your host and install the base k3s package.
...
<details>
  ...
  <ul>
    <li>
      By default, Kubernetes sets up a virtual network between the containers.
      This lets application components communicate with each other and
      (generally) the internet.
    </li>
    <li>
      We need an [ingress
      controller](https://www.nginx.com/resources/glossary/kubernetes-ingress-controller/)
      to control how we access our cluster from the outside. It's like adding a
      gate to a walled city. It allows access- but we can specify what kind of
      traffic can get in and where it goes in the cluster.
    </li>
  </ul>
</details>
