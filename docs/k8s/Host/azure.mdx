...
Your shell is connected to a subscription. When you create resources (groups/vm's/etc) in Azure they will be created in your current subscription.
Ensure that you connect to the subscription where you want to build your host.
...
- Azure generates an ssh key for the host and automatically adds it to your cloud shell. This way you can login without a password.

:::

Once the script presents you with additional options, it's safe to press < enter > and quit. You can run the helper script again at any time.

Copy the **myip** file into your host to reference later typing `yes` if asked to store fingerprint & entering the host password you just created.
...
<details>
  ...
  The cat command displays the contents of said file and the $() part means "run
  whatever command is in here". In the end, scp gets the azure user name and the
  IP of the host. The part after ':' is the folder and filename where scp saves
  the file.
</details>
