...
**Either:** select "bash" as the terminal type **-or-** ensure Bash is selected at the top of the terminal window.  
![azure bash](/img/bash.png)

:::tip Which subscription am I using?
Your shell is attached to a subscription which determines where commands run. You can check to see which subscription is selected with:
