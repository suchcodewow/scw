...
We can easily see the entire flow from front end users impacting back to exact log error message.

6. Disable the fraud service. On the end of the base URL, add:

```bash
/api/api/features/disable/fraud_service
```

7. Review the transactionAPI for decrease in failures. After a few minutes without user users, Dynatrace will automatically close the ticket.
