...
4. In the tokens screen, click "Generate Token" on the right side. Give it a name. It could be "Fullstack Workshop" or "Rick".
5. In the "search scopes" box, type "write api tokens", then select the option when it appears below.
   ![API Token](/img/token.jpg)
6. Click "Generate Token" at the bottom of the screen. Click Copy on the following screen. Make sure to **paste** your token before clicking done.
7. Click Done.
...
3. If it works, you'll return to the main menu.
4. Press _enter_ to stop Pepper for now.

### C) Deploy Dynatrace

1. Navigate to the "Dynatrace Hub" on the left side of Dynatrace (also under the "Manage" heading)
2. Select the big "OneAgent" panel, then "Download OneAgent" at the bottom of screen.
3. Pick Kubernetes from the most common deployment options.
   ![kubernetes](/img/deploy2.jpg)
4. Give it a friendly name like "CloudKubernetes" or "Morty".
5. Click "Create token" for Dynatrace Operator & then click "copy" near the bottom.
   ![copybutton](/img/deploy3.jpg)

6. In your cloud shell prompt (not Pepper), paste the contents

   ![Copy Command](/img/commands.jpg)

7. Press _enter_ a couple of times to ensure your cloud provider runs all 4 steps.
...
------ -----------                   -------
     5 dynatrace : Show Pods         5/5 pods READY
     6 dynatrace : Remove            tenant: kge67267
```
...
1. In your cloud shell, select the option to `download dempo apps yaml files`.
